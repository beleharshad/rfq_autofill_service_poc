...
_STEP_SUMMARY_SCHEMA_VERSION = "0.2"
...
class StepAnalysisService:
	...
	def analyze_step_job(self, job_id: str, step_input_path: Path) -> Dict[str, Any]:
		...
		if part_summary_path.exists():
			part_summary = json.loads(part_summary_path.read_text(encoding="utf-8-sig"))
			if self._should_regenerate_part_summary(part_summary):
				part_summary = self._generate_part_summary(canonical_step_path)
		...
		prompt = (
			_STEP_ANALYSIS_PROMPT
			.replace("{metadata_json}", json.dumps(metadata, indent=2))
			.replace("{geometry_json}", json.dumps(self._build_llm_geometry_payload(part_summary), indent=2))
			.replace("{candidate_json}", json.dumps(self._build_llm_candidate_payload(part_summary, deterministic), indent=2))
		)
	...
	def _should_regenerate_part_summary(self, part_summary: Dict[str, Any]) -> bool:
		"""Return True when a cached STEP summary should be rebuilt."""
		if str(part_summary.get("schema_version") or "") != _STEP_SUMMARY_SCHEMA_VERSION:
			return True

		selected_body = part_summary.get("selected_body") or {}
		if str(selected_body.get("extraction_method") or "") == "bbox":
			warning = str(selected_body.get("analysis_warning") or "")
			if "Feature extraction produced no turned segments" in warning:
				return True
	...
	def _build_feature_summary(
		...
		return {
			"schema_version": _STEP_SUMMARY_SCHEMA_VERSION,
	...
	def _build_deterministic_result(self, step_path: Path, part_summary: Dict[str, Any]) -> Dict[str, Any]:
		...
		def _pick_primary_id() -> Optional[float]:
			if not segments or not od_in:
				return ids[0] if ids else None

			min_meaningful_id = max(float(od_in) * 0.02, 0.05)
			min_meaningful_span = max(float(length_in or 0.0) * 0.01, 0.02)
			coverage_by_id: Dict[float, float] = {}

			for seg in segments:
				seg_id = float(seg.get("id_diameter") or 0.0)
				seg_od = float(seg.get("od_diameter") or 0.0)
				seg_span = abs(float(seg.get("z_end") or 0.0) - float(seg.get("z_start") or 0.0))
				if seg_id <= 0.0 or seg_od <= 0.0 or seg_span <= 0.0:
					continue
				if seg_id >= seg_od:
					continue
				coverage_by_id[seg_id] = coverage_by_id.get(seg_id, 0.0) + seg_span

			meaningful = [
				(seg_id, span)
				for seg_id, span in coverage_by_id.items()
				if seg_id >= min_meaningful_id and span >= min_meaningful_span
			]
			if meaningful:
				meaningful.sort(key=lambda item: (item[1], item[0]), reverse=True)
				return meaningful[0][0]

			if coverage_by_id:
				ranked = sorted(coverage_by_id.items(), key=lambda item: (item[1], item[0]), reverse=True)
				return ranked[0][0]

			return ids[0] if ids else None

		id_in = _pick_primary_id()
		...
		if ids:
			cross_checks.append(
				f"Detected {len(ids)} positive bore diameter candidate(s) from STEP geometry; id_in uses the dominant span-supported bore and max_id_in uses the largest."
			)
		else:
